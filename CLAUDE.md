...
cargo test -- --ignored              # Run mux integration tests (requires tmux or cmux)
...
Stagent is an interactive TUI for reviewing unstaged git changes hunk-by-hunk, staging selectively, and collecting edit/comment feedback. It prefers cmux, then tmux, then full-screen fallback for spawned editor/review sessions.
...
5. **Editor** (`editor.rs`): opens `$EDITOR` in cmux, tmux, or the current terminal and polls the selected backend for close detection
...
- **Close detection is backend-specific** — tmux still uses `list-panes -a`; cmux uses `read-screen` existence checks
...
- `mux.rs` — backend detection and split lifecycle for cmux/tmux
- `editor.rs` — editor session lifecycle, tempfile prep, result parsing
